...
public class FrameTrainingSetToLibsvm {
	...
	public FrameTrainingSetToLibsvm(File featureFile, File instanceFile, File libsvnFile, File labelFile, File gazetteerFile) throws IOException {
		...
		FrameFeatureExtraction frameFeatureExtraction = new FrameFeatureExtraction(featureIndex, exampleList, gazetteerMap);
		for (int i = 0; i < exampleList.size(); i++) {

			String[] example = exampleList.get(i);
			logger.trace(example.length + "\t" + Arrays.toString(example));

			int label = labelIndex.put(example[0]);
			logger.trace(example[0] + "\t" + label);
			//SortedSet<Integer> set = frameFeatureExtraction.extract(i);
			//String libsvmExample = label + " " + setToString(set);
			String libsvmExample = label + " " + frameFeatureExtraction.extractVector(i);
	...
	public static void main1(String[] args) throws Exception {
		//java -cp dist/dirha.jar org.fbk.cit.hlt.dirha.SpreadExampleListSheetToRoleTrainingSet
		...
		if (args.length != 2) {
			logger.error("java -Dfile.encoding=UTF-8 -cp dist/dirha.jar org.fbk.cit.hlt.dirha.FrameTrainingSetToLibsvm fin gaz");
			System.exit(-1);
		}


		File featureFile = new File(args[0] + ".feat");
		File exampleFile = new File(args[0]);
		File libsvmFile = new File(args[0] + ".svm");
		File labelFile = new File(args[0] + ".label");


		File gazetteerFile = new File(args[1]);
		new FrameTrainingSetToLibsvm(featureFile, exampleFile, libsvmFile, labelFile, gazetteerFile);
	}
