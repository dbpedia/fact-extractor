...
@parser::members {
    public List<DateEntity> results = new ArrayList<DateEntity>();
    public DateEntity result;
}
...
date_or_time
	...
	result.type = DateEnum.TIMEX_DATE;
        ...
        result = new DateEntity();
	    result.type = DateEnum.TIMEX_DATE;
        result.value = String.format("%02d", $month.i) + ":_";
	    results.add(result);
      result = new DateEntity();
	...
	| week_number
		{
			...
			result.value = String.format("%d", $week_number.i);
			results.add(result);
		}
...
time_number returns [String s]
	...
	*/
	| a=number b=number
	  { $s = String.format("%02d", $a.i) + ":" +
                 String.format("%02d", $b.i) + ":_"; }
	| a=number ':' b=number
	  { $s = String.format("%02d", $a.i) + ":" +
                 String.format("%02d", $b.i) + ":_"; }
	...
	| a=number MENO b=number
	  { $s = String.format("%02d", ($b.i+23) % 24) + ":" +
                 String.format("%02d", (60-$a.i)) + ":_"; }
	| number E UN QUARTO
	  { $s = String.format("%02d", $number.i) + ":" + 15 + ":_"; }
	| UN QUARTO DOPO LE number
	  { $s = String.format("%02d", $number.i) + ":" + 15 + ":_"; }
	| number E TRE QUARTI
	  { $s = String.format("%02d", $number.i) + ":" + 45 + ":_"; }
	| UN QUARTO ALLE number
	  { $s = String.format("%02d", ($number.i+23) % 24) + ":" + 45 + ":_"; }
	| number MENO UN QUARTO
	  { $s = String.format("%02d", ($number.i+23) % 24) + ":" + 45 + ":_"; }
	| number E MEZZO
	  { $s = String.format("%02d", $number.i) + ":" + 30 + ":_"; }
	| threedignum
	  { $s = String.format("%02d", ($threedignum.i / 100)) + ":" +
	  		 String.format("%02d", ($threedignum.i % 100)) + ":_" ; }
	| ORE timenum
	  { $s = String.format("%02d", ($timenum.i / 100)) + ":" +
	  		 String.format("%02d", ($timenum.i % 100)) + ":_" ; }
	| timenum
	  { $s = String.format("%02d", ($timenum.i / 100)) + ":" +
	  		 String.format("%02d", ($timenum.i % 100)) + ":_" ; }
	| number
...
date returns [String s]
	// 16 novembre
	: number month
	  { $s = String.format("%02d", $month.i) + ":" +
                 String.format("%02d", $number.i); }
	// primo novembre
	| PRIMO month
	{ $s = String.format("%02d", $month.i) + ":01"; }
	// 16/11
  | a=number SLASH b=number
  { $s = String.format("%02d", $b.i) + ":" +
               String.format("%02d", $a.i); }
	// 16 di gennaio
  ...
  { $s = String.format("%02d", $month.i) + ":" +
	...
	| number maybedi QUESTO MESE
	  { $s = "_:" + String.format("%02d", $number.i); }
	// novembre
  | month
  { $s = String.format("%02d", $month.i) + ":_"; }
	;
...
timecomponents returns [String s]
	: a=number ORE b=number MINUTI and c=number SECONDI
	  { $s = String.format("%02d", $a.i) + ":" +
                 String.format("%02d", $b.i) + ":" +
                 String.format("%02d", $c.i) ; }
	| a=number ORE and b=number MINUTI
	  { $s = String.format("%02d", $a.i) + ":" +
                 String.format("%02d", $b.i) + ":_"; }
	| a=number ORE and b=number SECONDI
	  { $s = String.format("%02d", $a.i) + ":_:" +
                 String.format("%02d", $b.i) ; }
	| a=number MINUTI and b=number SECONDI
	  { $s = "_:" + String.format("%02d", $a.i) + ":" +
                 String.format("%02d", $b.i) ; }
	| number ORE
          { $s = String.format("%02d:_:_", $number.i) ; }
	...
	| UN ORA
          { $s = "01:_:_" ; }
	| number MINUTI
          { $s = String.format("_:%02d:_", $number.i) ; }
	| number SECONDI
          { $s = String.format("_:_:%02d", $number.i) ; }
	;
